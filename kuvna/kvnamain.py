...
form_class = uic.loadUiType("vna_frontpanel.ui")[0]
...
class MyWindow(QMainWindow, form_class):
    ...
    def initUI(self):
        ...
        self.ckbox_s11.setCheckState(self.s11chkFlag)
    ...
    def initSignal(self):      
        ...
        # 체크박스
        self.ckbox_s11.stateChanged.connect(self.changeS11)
        self.ckbox_s21.stateChanged.connect(lambda: print("S21 체크박스 변경"))
        self.ckbox_s12.stateChanged.connect(lambda: print("S12 체크박스 변경"))
        self.ckbox_s22.stateChanged.connect(lambda: print("S22 체크박스 변경"))
    
        self.ckbox_open.stateChanged.connect(lambda: print("Open 체크박스 변경"))
        self.ckbox_load.stateChanged.connect(lambda: print("Load 체크박스 변경"))
        self.ckbox_short.stateChanged.connect(lambda: print("Short 체크박스 변경"))
        self.ckbox_thru.stateChanged.connect(lambda: print("Thru 체크박스 변경"))
    
        # 콤보박스
        self.cbbox_view.currentIndexChanged.connect(self.changeViewID)
        self.cbbox_format.currentIndexChanged.connect(lambda index: print(f"Format 콤보박스 선택: {self.cbbox_format.currentText()}"))
    ...
    def show_graph(self):
        if self.dataFrag == True:
            ...
            self.graph_viewer.canvas.axes=self.graph_viewer.canvas.figure.add_subplot(1,1,1)
            self.graph_viewer.canvas.axes.plot(self.uvna.freq_vec, self.uvna.s11_abs_db,label = "S11")
            self.graph_viewer.canvas.axes.plot(self.uvna.freq_vec, self.uvna.s21_abs_db,label = "S21")
            if self.port == 2:
                self.graph_viewer.canvas.axes.plot(self.uvna.freq_vec, self.uvna2.s22_abs_db,label = "S22")
                self.graph_viewer.canvas.axes.plot(self.uvna.freq_vec, self.uvna2.s12_abs_db,label = "S12")
    ...
    def show_graph_2(self):
        ...
        if self.dataFrag == True:
            ...
            self.graph_viewer.canvas.figure.clear()
            self.graph_viewer.canvas.draw()
            self.graph_viewer.canvas.axes=self.graph_viewer.canvas.figure.add_subplot(2,1,1)
            self.graph_viewer.canvas.axes.plot(self.uvna.freq_vec, self.uvna.s11_abs_db,label = "S11")
            self.graph_viewer.canvas.axes.plot(self.uvna.freq_vec, self.uvna.s21_abs_db,label = "S21")
            if self.port == 2:
                self.graph_viewer.canvas.axes.plot(self.uvna.freq_vec, self.uvna2.s22_abs_db,label = "S22")
            ...
            self.graph_viewer.canvas.axes=self.graph_viewer.canvas.figure.add_subplot(2,1,2)
            self.graph_viewer.canvas.axes.plot(self.uvna.freq_vec, self.uvna.s11_ang,label = "S11")
            self.graph_viewer.canvas.axes.plot(self.uvna.freq_vec, self.uvna.s21_ang,label = "S21")
            if self.port == 2:
                self.graph_viewer.canvas.axes.plot(self.uvna.freq_vec, self.uvna2.s22_ang,label = "S22")
                self.graph_viewer.canvas.axes.plot(self.uvna.freq_vec, self.uvna2.s12_ang,label = "S12")
            self.graph_viewer.canvas.axes.legend()
            self.graph_viewer.canvas.axes.set_xlabel('frequency(MHz)')
    ...
    def show_graph_3(self):
        ...
        if self.dataFrag == True:
            ...
            self.graph_viewer.canvas.figure.clear()
            self.graph_viewer.canvas.draw()
            self.graph_viewer.canvas.axes=self.graph_viewer.canvas.figure.add_subplot(2,2,1)
            self.graph_viewer.canvas.axes.plot(self.uvna.freq_vec, self.uvna.s11_abs_db,label = "S11")
            if self.port == 2:
                self.graph_viewer.canvas.axes.plot(self.uvna.freq_vec, self.uvna2.s22_abs_db,label = "S22")
            self.graph_viewer.canvas.axes.legend()
            ...
            self.graph_viewer.canvas.axes=self.graph_viewer.canvas.figure.add_subplot(2,2,2)
            self.graph_viewer.canvas.axes.plot(self.uvna.freq_vec, self.uvna.s21_abs_db,label = "S21")
            if self.port == 2:
                self.graph_viewer.canvas.axes.plot(self.uvna.freq_vec, self.uvna2.s12_abs_db,label = "S12")
            self.graph_viewer.canvas.axes.legend()
            ...
            self.graph_viewer.canvas.axes=self.graph_viewer.canvas.figure.add_subplot(2,2,3)
            self.graph_viewer.canvas.axes.plot(self.uvna.freq_vec, self.uvna.s11_ang,label = "S11")
            if self.port == 2:
                self.graph_viewer.canvas.axes.plot(self.uvna.freq_vec, self.uvna2.s22_ang,label = "S22")
            self.graph_viewer.canvas.axes.legend()
            ...
            self.graph_viewer.canvas.axes=self.graph_viewer.canvas.figure.add_subplot(2,2,4)
            self.graph_viewer.canvas.axes.plot(self.uvna.freq_vec, self.uvna.s21_ang,label = "S21")
            if self.port == 2:
                self.graph_viewer.canvas.axes.plot(self.uvna.freq_vec, self.uvna2.s12_ang,label = "S12")
            self.graph_viewer.canvas.axes.legend()
...
#        self.graph_viewer.canvas.axes.legend()
    ...
    def changeS11(self):
        self.s11chkFlag = self.ckbox_s11.checkState()
        self.displayGraph()
